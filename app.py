...
from backend.vector_store import store_embeddings, search_embeddings, delete_session_collection
...
async def upload_pdf(
    ...
    try:
        ...
        store_embeddings(chunks, session_id=session_id)   # FIX A: pass session_id
...
async def ask_question(request: Askquestion, session_id: Optional[str] = Cookie(default=None)):
    ...
    try:
        # FIX A + C: pass session_id so we search only this user's data;
        # search_embeddings now returns top 3 chunks merged into one string.
...
async def delete_session(response: Response, session_id: Optional[str] = Cookie(default=None)):
    ...
    delete_session_collection(session_id)   # FIX A: clean up ChromaDB collection
